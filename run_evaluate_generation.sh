...
task="evaluate_generation"

prefix_task=" task="
prefix_dataset_name=" dataset_name="

prefix_augmentation_method=" evaluate_generation.method="
prefix_distance=" evaluate_generation.distance="

prefix_feature_extractor=" evaluate_generation.feature_extractor.estimator="
estimators=("LITE")
...
to_skip_for_now=("PLAID" "Tools" "AsphaltPavementType" "FiftyWords" "AsphaltRegularity" "KeplerLightCurves" "PhoneHeartbeatSound" "HandOutlines" "Phoneme" "ACSF1" "DiatomSizeReduction")
...
for dataset_name in "${dataset_names[@]}"; do
    ...
    for estimator in "${estimators[@]}"; do
        ...
        for method in $methods; do
            ...
            if [ "$values" == "[]" ]; then
                running_experiment_name=$task"_"$dataset_name"_"$method"_"$estimator
                ...
                jq -c --arg method "$method" '.[$method][]' "$json_file" | while read -r params; do
                    ...
                    if [ -z "$extra_param" ]; then
                        running_experiment_name=$task"_"$dataset_name"_"$method"_"$distance"_"$estimator
                        args=$running_experiment_name
                        args=$args$prefix_task$task
                        args=$args$prefix_dataset_name$dataset_name
                        args=$args$prefix_augmentation_method$method
                        ...
                        space=" "
                        equal="="
                        prefix_extra_key="evaluate_generation."

                        running_experiment_name=$task"_"$dataset_name"_"$method"_"$distance"_"$extra_key"_"$extra_param"_"$estimator
                        args=$running_experiment_name
                        args=$args$prefix_task$task
                        args=$args$prefix_dataset_name$dataset_name
                        args=$args$prefix_augmentation_method$method
